...
(function($) {
  ...
  $.fn.additionalContentLoader = function (_contentName) {
    ...
    // Content already loaded? Just go ahead!
    if ($('#' + _fileName).length > 0) {
      $.bbq.pushState('#/' + _contentName);
      return;
    }
  ...
  var
    ...
    cols_click_handler = function (evt) {
      ...
      if (!$col.hasClass('opened-column')) {
        // Stop all videos
        $('#additional-content-wrapper').videoControl('stop');
  ...
  //$(function() {
    var
      ...
      // Menu & Event Listener
      $menu = $('#primary-menu, footer nav, #additional-content-wrapper>nav')
   ...
   */
  $('#primary-menu').affix({offset: 96});

  /**
   * Smooth Scroll
   */
  var smoothScroll = function (_tgt) {
    var $tgt = $('#' + _tgt);
    if ($tgt.length > 0) {
      $.smoothScroll({
        scrollTarget: '#' + _tgt,
        ...
        afterScroll: function () {
          $tgt.attr('tabindex','0').focus().removeAttr('tabindex');
        }
      });
    }
  }
   ...
   * History keeper
   */
  , hashchange_window_handler = function (evt) {
    var _action = evt.action || 0
      , _id = evt.id || 0;
    switch (_action) {
      case 'additionalContent':
        $.fn.additionalContentLoader(_id);
        break;

      case 'init':
        menu_controller(location.hash);
        break;

      default:
        var _tgt = location.hash.replace(/^#\/?/,'');
        if (_tgt.length > 0
        &&  $('#' + _tgt).length == 0) {
          $.fn.additionalContentLoader(_tgt);
          break;
        }
        if (/^video/.test(_tgt)) {
          menu_controller(_tgt);
        }
        smoothScroll(_tgt);
        break;
    }
  };
  $(window).on('hashchange', hashchange_window_handler);
  $(window).trigger({
    type: 'hashchange',
    action: 'init'
  });
